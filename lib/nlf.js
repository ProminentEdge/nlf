...
function parseInstalled(data, options, callback) {
	 ...
	 */
	(function parseModuleData(moduleData, output, skipCurrentModule) {
		...
		if (!output.hasOwnProperty(moduleData._id)) {
			...
			createModule(moduleData, function (err, module) {
				...
				// add this module to the output object/collection, but skip based on skipCurrentModule. Meant to be used to skip
				// the root module. The output should not include this own project
				if (!skipCurrentModule) {
				...
				for (name in dependencies) {
					if (dependencies.hasOwnProperty(name)) {
						if (typeof dependencies[name] === 'object') {
							...
							parseModuleData(dependencies[name], output, false);
						} else {
...
function createModule(moduleData, callback) {
	...
	if (repository.split('/').length >= 4) {
		// examples:
		//   ssh://git@github.com/isaacs/abbrev-js
		//   https://github.com/jshttp/accepts
		//   http://www.site.com/org/repo
		githubUser = repository.split('/')[3]
	}
	var author = (moduleData.author || {}).name || githubUser || '(none)';
	var description = moduleData.description || '(none)';
	var directory = moduleData.path;
	var id = createId(moduleData);
	var name = moduleData.name || id;
	var version = moduleData.version || '0.0.0';
	var module = new Module(id, name, version, directory, repository, undefined, author, description);
